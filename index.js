  ...
  return String(text || '')
    ...
    .map(line => {
      ...
      if (nums.length >= 2) {
        ...
        return line.replace(/ワイド.*$/g, `ワイド ${nums[0]}${WIDE_SEPARATOR}${nums[1]}${amountText}`);
      }

      return line
        .replace(/(\d{1,2})\s*[-‐-‒–—―−ーｰ－]\s*(\d{1,2})/g, `$1${WIDE_SEPARATOR}$2`)
        .replace(/(\d{1,2})\s+－\s+(\d{1,2})/g, `$1${WIDE_SEPARATOR}$2`);
    })
    .join('\n');
}
...
ワイドは「11－14」のように全角ハイフンで表示してください。
